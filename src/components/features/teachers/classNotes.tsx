...
import { deteleClassNotes, getTecaherClassNotesList } from '@/hooks/apiCalls/teacher'
import { useDispatch, useSelector } from 'react-redux'
import { setClassNotesData } from '@/redux/classNoteSlice'
...
const TeacherClassNotes = ({ loading, setLoading }: any) => {
  ...
  const handleDeleteNote = useCallback(async (noteId: string, title: string) => {
    Alert.alert(
      'Delete Note',
          ...
          onPress: async () => {
            ...
            try {
              ...
              if (res?.success === true) {
                ...
                Alert.alert('Failed', res?.message || 'Failed to delete note')
              }
            } catch (error) {
        ...
        <View style={styles.noteLeftSection}>
          ...
          {item.chapter && (
            <Text style={styles.noteChapter}>
              <Ionicons name="book-outline" size={14} color="#6B7280" /> {item.chapter}
            </Text>
          )}
  ...
  ), [selectedClass, selectedSection, selectedSubject, classOptions, sectionOptions, subjectOptions, loading, handleClassNotesList])
